...
test.describe('Landing Page', () => {
  ...
  test('has correct title and meta description', async ({ page }) => {
    ...
    await expect(metaDescription).toHaveAttribute(
      'content',
      /AI‑powered résumé, portfolio & interview coach/
    )
  })
  ...
  test('renders hero section correctly', async ({ page }) => {
    // Check main heading
    await expect(
      page.getByRole('heading', { name: /Launch your career in minutes/i })
    ).toBeVisible()

    // Check description
    await expect(
      page.getByText(/AI‑generated portfolio, résumé & real‑time interview coach/)
    ).toBeVisible()
