...
export async function chatLLM(
  ...
  if (!apiKey) {
    throw new Error(
      `Missing API key for ${provider}. Please set ${provider === 'groq' ? 'GROQ_API_KEY' : 'OPENAI_API_KEY'} in your environment variables.`
    )
  }
  ...
  const safeOpts =
    provider === 'groq' ? (({ logprobs, logit_bias, n, ...rest }) => rest)(opts as any) : opts
