...
export function extractDomainsFromProfile(profile: Profile): string[] {
  ...
  if (profile.skills) {
    profile.skills.forEach(skill => {
      ...
      if (
        ['node.js', 'python', 'java', 'go', 'rust', 'api', 'database', 'sql'].some(tech =>
          skillLower.includes(tech)
        )
      ) {
        domains.add('Backend Development')
      ...
      if (
        ['figma', 'sketch', 'adobe', 'design', 'prototyping'].some(tech =>
          skillLower.includes(tech)
        )
      ) {
        domains.add('UX/UI Design')
      ...
      if (
        ['python', 'r', 'machine learning', 'data analysis', 'pandas', 'numpy'].some(tech =>
          skillLower.includes(tech)
        )
      ) {
        domains.add('Data Science')
  ...
  if (profile.education) {
    profile.education.forEach(edu => {
      if (edu.degree) {
        ...
        if (
          degree.includes('data') ||
          degree.includes('statistics') ||
 ...
 */
export async function getDynamicDomains(): Promise<
  { domain: string; title: string; isCustom: boolean }[]
> {
...
export function getRandomQuestionSet(domain: Domain): Question[] {
  ...
  if (!questionSet || !questionSet.questions || questionSet.questions.length === 0) {
    ...
    if (
      ...
      backendQuestions.questions.length === 0
    ) {
