...
Ensure the JSON is valid and complete. Do not include any text outside the JSON object.`
    ...
    try {
      ...
      const missingSkills = skillsInJob
        .filter(
          (jobSkill: string) =>
            !skillsInProfile.some(
              (skill: string) =>
                skill.toLowerCase().includes(jobSkill.toLowerCase()) ||
                jobSkill.toLowerCase().includes(skill.toLowerCase())
            )
      ...
      const matchPercentage =
        skillsInJob.length > 0 ? Math.round((matchingSkills.length / skillsInJob.length) * 100) : 0
      ...
      analysis = {
        ...
        recommendations: {
          ...
          skills: missingSkills
            .slice(0, 5)
            .map((skill: string) => `Add ${skill} to skills section`),
