  ...
  try {
    ...
    try {
      ...
      const response = await chatLLM('groq', 'llama3-8b-8192', [
        {
          role: 'system',
          content:
            'You are a strict interviewer evaluating the candidate\'s answer. Respond with ONLY a JSON object containing \'score\' (0-10) and \'tips\' (array of strings). Example: {"score": 7, "tips": ["Be more specific", "Add examples"]}',
        },
      ...
      try {
        ...
        const scoreMatch =
          content.match(/score[\"']?\s*:\s*[\"']?(\d+)/i) || content.match(/(\d+)\/10/)
